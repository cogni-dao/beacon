 ...
 * Purpose: Read-only presentation of one growth campaign — title, status,
 *   independent engagement KPI (score vs target), and the broadcast/snapshot
 *   ground-truth that produced it.
...
function statusBadge(row: CampaignLensRow): ReactElement {
  if (!row.resolved) {
    return (
      ...
      <CardContent className="flex flex-col gap-3">
        ...
        <dl className="grid grid-cols-3 gap-2 text-xs">
          ...
          <div>
            ...
            <dd className="font-medium tabular-nums">{row.postedBroadcasts}</dd>
          </div>
          <div>
