...
-- Handwritten appendix: RLS FORCE is outside drizzle's DDL scope (same as
-- 0027_rls_epoch_coverage and upstream node-template 0029-0031). ENABLE + FORCE
-- with NO policy = deny-all, fail-closed for app_user; the finalize/distribution
-- and identity paths run on the service role (BYPASSRLS) and are unaffected.
